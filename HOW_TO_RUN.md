...
- No internet connection is required (no CDN dependencies anymore).

## First-run walkthrough

1. **Add a piece.** Click *+ Add piece* and choose a `.mid` / `.midi` file. It's stored as a blob in IndexedDB and parsed in-browser.
...
3. **Practice.** Click a section (or one from the "Due today" queue). The notes fall Synthesia-style over a keyboard. Connect a MIDI keyboard, or click the on-screen keys, or toggle *Computer keys* and use `A…'` (with `Z`/`X` to shift octave).
...
`L` listen (synth preview) · `R` restart run · `1`–`4` rate · `Esc` stop · `M` metronome · `T` pause timer · `D` dark mode. (The opt-in *Computer keys* mode owns the letter keys while it's on.)
