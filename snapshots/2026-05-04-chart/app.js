...
import {
  ...
  exportLibrary,
  importLibrary,
  // Item 12c — practice-history chart
  listAllRepLogs,
} from './db.js';
import {
  // SM-2 surface (item 6)
...
const APP_VERSION = '0.12.2'; // roadmap item 12c — practice-history chart
...
async function hydrateFromStorage() {
  try {
    ...
    if (stored.length > 0) {
      setStatus(
        stored.length === 1
          ? `Loaded 1 saved piece \xb7 v${APP_VERSION}`
          : `Loaded ${stored.length} saved pieces \xb7 v${APP_VERSION}`,
      );
    } else {
      setStatus(`Ready \xb7 v${APP_VERSION}`);
    }
    ...
    if (queueState && queueState.sections.length > 0) {
      ...
      if (dueCount > 0) {
        setStatus(
          dueCount === 1
            ? `1 section due for review today \xb7 v${APP_VERSION}`
            : `${dueCount} sections due for review today \xb7 v${APP_VERSION}`,
        );
      }
    }
  } catch (err) {
    ...
    setStatus(
      `Ready (storage unavailable — pieces won't persist) \xb7 v${APP_VERSION}`,
    );
  }
}
